...
Caret.app starts pinned Screenpipe 0.4.50 on port 3031 (clipboard history on) when that port is free, and does not start a second recorder when something is already listening there. It writes `.local/screenpipe-lease.json`. The built app Info.plist carries `CaretProjectRoot` so the supervisor, skills, memories, and notes can find this repository. `python3 -m caret` can then ask for last-N windows, minutes, or clipboard, newest first.
