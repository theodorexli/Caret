...
Five upstreams are selected: KeyType and GhostType feed one native input system, Computer Use Jev handles native actions, Skyvern handles browser actions and Screenpipe handles history. Caret.app launches the pinned Screenpipe CLI; the other three do not run in the default starter. Changing a pin, a submodule SHA, or `.gitmodules` without the others fails the source check. Preserve upstream names, licenses and authorship.
